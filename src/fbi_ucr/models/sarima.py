"""SARIMA model implementation for inference."""
...
class SARIMAPredictor(BasePredictor):
    """SARIMA time series predictor with seasonal components."""
    ...
    def fit(self, y: pd.Series, X: Optional[pd.DataFrame] = None) -> "SARIMAPredictor":
        """Fit SARIMA model."""
        self.model = SARIMAX(
            y,
            order=self.order,
    ...
    def _extract_components(self, predicted: np.ndarray) -> ComponentBreakdown:
        """Extract trend component from SARIMA predictions.

        SARIMA includes seasonal differencing but doesn't expose decomposed
        components like Prophet. We calculate trend from predictions.
